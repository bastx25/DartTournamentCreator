...
export async function getTPlayers(id: number): Promise<TournamentPlayerDto[]> {
  const response = await axios.get<TournamentPlayerDto[]>(
...
export async function getBraketsByTournamentId(
  tournamentId: number,
): Promise<BraketDto[]> {
  const response = await axios.get<BraketDto[]>(
    `/api/tournaments/${tournamentId}/brakets`,
  );
